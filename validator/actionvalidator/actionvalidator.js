...
module.exports = function(core) {
	...
	*/
	...
	function validate(action, valid, callback) {
		...
		if (!result.status) {
			log.e("Error: invalid action", JSON.stringify(result), JSON.stringify(action));
	...
	var handlers = {
		...
		room: function(action, callback) {
			...
			if (action.room.guides.clearTime === "NOW") action.room.guides.clearTime = new Date().getTime();
			callback();
		}
	};
